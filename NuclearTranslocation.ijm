...
for (i_files=0; i_files<lengthOf(Filelist); i_files++) {
	if (endsWith(Filelist[i_files], extension)) { // Opens .nd or .nd2 or .tif 
		...
		// if Z-stack the user is asked on which slices to perform the analysis 
		// maximum Z-projection will be applied
		if (slices>1) {
		...
		// if a file containing cell ROIs exist, it is loaded
		...
		// cells ROIs are saved
		roiManager("deselect");
		roiManager("save", dir_output+subname+"_CellROIs.zip");
		
		// measure of the intensity of the translocated protein
		// in the whole cell
		...
		Area=newArray(NumbCell);
		TotMean=newArray(NumbCell);
		NucMean=newArray(NumbCell);
		// fills image name, cell area and translocated protein intensity in the cell
		...
		// Nuclei detection		
		// If nucleus ROIs exists from previous analysis, and no cell has been added in this current analysis, 
		// the Nuclei ROIs are used
		if (File.exists(dir_output+subname+"_NucleiROIs.zip") && !cell_added) { 
			roiManager("reset");
			roiManager("open", dir_output+subname+"_NucleiROIs.zip");			
		}
		// otherwise the user is asked to choose a threshold to 
		// create the binary image for the nuclei
		else {
			...
			// analyze particles in each cell
		...
		// measures the translocated protein intensity in the cell in the nucleus
		selectWindow("Translocated_Prot");
		roiManager("deselect");
		roiManager("measure");
